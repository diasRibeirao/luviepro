...
const base=Platform.OS!=='web'&&metroHost&&configuredBase?.includes('localhost')?`http://${metroHost}:3333/api`:Platform.OS==='web'&&webHost&&configuredBase?.includes('localhost')?`http://${webHost}:3333/api`:(configuredBase??(webHost?`http://${webHost}:3333/api`:'http://localhost:3333/api'));
let token='';
let refreshToken='';
