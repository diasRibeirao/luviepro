...
const allowInsecureApi=process.env.EXPO_PUBLIC_ALLOW_INSECURE_API==='true';
function apiBase(){
  ...
  if(configured){if(!development&&!allowInsecureApi&&!configured.startsWith('https://'))throw new Error('EXPO_PUBLIC_API_URL deve utilizar HTTPS na build de produção');return configured;}
...
async function hydrateNativeAuth(){
  ...
  if(token&&refreshToken&&session)return true;
  if(nativeHydratePromise)return nativeHydratePromise;
  nativeHydratePromise=(async()=>{try{const raw=await readAuth();if(!raw)return false;const parsed=JSON.parse(raw);if(!parsed?.session||!parsed?.token||!parsed?.refreshToken)return false;session=parsed.session;token=parsed.token;refreshToken=parsed.refreshToken;return true;}catch{return false;}finally{nativeHydratePromise=undefined;}})();
  return nativeHydratePromise;
}
export async function restoreSession(){try{if(Platform.OS==='web'){const raw=await readAuth();if(raw){const parsed=JSON.parse(raw);session=parsed?.session;}token='';refreshToken='';return renew();}return hydrateNativeAuth();}catch{clearSession();return false;}}
...
export async function api<T=unknown>(path:string,init:RequestInit={},retry=true):Promise<T>{
  ...
  if(Platform.OS!=='web'&&!authPath&&!token)await hydrateNativeAuth();
...
export async function apiForm<T=unknown>(path:string,form:FormData,retry=true):Promise<T>{
  let res:Response;
  if(Platform.OS!=='web'&&!token)await hydrateNativeAuth();
