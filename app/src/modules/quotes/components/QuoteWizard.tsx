...
export default function QuoteWizard({embedded=false,onClose,onSaved,calculatorDraft}:{embedded?:boolean;onClose?:()=>void;onSaved?:()=>void;calculatorDraft?:CalculatorQuoteDraft}={}){
  ...
  const syncStageDays=(item:ServiceItem,stages:StageLine[])=>({...item,stages,days:String(projectDaysFromStages(stages,Number(item.days)||1))});
  const updateStage=(itemIndex:number,stageIndex:number,key:'description'|'duration',value:string)=>setServiceItems(current=>current.map((item,i)=>i===itemIndex?syncStageDays(item,item.stages.map((stage,j)=>j===stageIndex?{...stage,[key]:value}:stage)):item));
  const addStage=(itemIndex:number)=>setServiceItems(current=>current.map((item,i)=>i===itemIndex?syncStageDays(item,[...item.stages,{description:'',duration:'1 dia'}]):item));
  const removeStage=(itemIndex:number,stageIndex:number)=>setServiceItems(current=>current.map((item,i)=>i===itemIndex?syncStageDays(item,item.stages.filter((_,j)=>j!==stageIndex)):item));
  ...
  const next=()=>{
    ...
    if(step===2){
      serviceItems.forEach((item,index)=>{
        ...
        item.stages.forEach((stage,stageIndex)=>{if(!stage.description.trim())nextErrors[`service-${index}-stage-${stageIndex}`]='Informe a descrição da etapa.';});
      });
...
function StageEditor({stages,itemIndex,errors,update,add,remove}:{stages:StageLine[];itemIndex:number;errors:Record<string,string>;update:(itemIndex:number,stageIndex:number,key:'description'|'duration',value:string)=>void;add:(itemIndex:number)=>void;remove:(itemIndex:number,stageIndex:number)=>void}){const brand=useTenantBrand();return <View style={s.stageSummary}><View style={s.stageSummaryHead}><View><Text style={s.lineTitle}>Etapas do serviço</Text><Text style={s.lineHint}>Personalize as etapas somente para este orçamento.</Text></View></View>{stages.length===0?<Text style={s.noLines}>Nenhuma etapa selecionada. Você pode adicionar somente as etapas necessárias.</Text>:stages.map((stage,index)=><View key={index} style={s.stageEditRow}><View style={[s.stageIndex,{backgroundColor:brand.primarySoft}]}><Text style={[s.stageIndexText,{color:brand.primary}]}>{index+1}</Text></View><View style={{flex:1}}><TextInput value={stage.description} onChangeText={value=>update(itemIndex,index,'description',value)} placeholder="Descrição da etapa" style={[s.stageInput,errors[`service-${itemIndex}-stage-${index}`]&&s.inputError]}/>{errors[`service-${itemIndex}-stage-${index}`]?<Text style={s.errorText}>{errors[`service-${itemIndex}-stage-${index}`]}</Text>:null}</View><TextInput value={stage.duration??''} onChangeText={value=>update(itemIndex,index,'duration',value)} placeholder="1 dia" style={s.stageDurationInput}/><Pressable accessibilityLabel={`Excluir etapa ${index+1}`} onPress={()=>remove(itemIndex,index)} style={s.lineRemove}><Ionicons name="trash-outline" size={16} color={theme.danger}/></Pressable></View>)}<Pressable onPress={()=>add(itemIndex)} style={s.stageAdd}><Ionicons name="add-circle-outline" size={16} color={brand.primary}/><Text style={[s.miniAddText,{color:brand.primary}]}>Adicionar etapa</Text></Pressable></View>}
function LineGroup({title,hint,group,lines,itemIndex,update,add,remove,errors}:{title:string;hint?:string;group:'team'|'variable'|'fixed';lines:CostLine[];itemIndex:number;update:(i:number,g:'team'|'variable'|'fixed',l:number,k:keyof CostLine,v:string)=>void;add:(i:number,g:'team'|'variable'|'fixed')=>void;remove:(i:number,g:'team'|'variable'|'fixed',l:number)=>void;errors?:Record<string,string>}){const brand=useTenantBrand();return <View style={s.lineGroup}><View style={s.lineGroupHead}><View style={s.lineTitleRow}><Text style={s.lineTitle}>{title}</Text>{hint&&<Text style={s.lineHint}>({hint})</Text>}</View></View>{lines.length===0?<Text style={s.noLines}>Nenhum item adicionado.</Text>:lines.map((line,lineIndex)=>{const labelError=errors?.[`service-${itemIndex}-${group}-${lineIndex}-label`],valueError=errors?.[`service-${itemIndex}-${group}-${lineIndex}-value`];return <View key={`${group}-${lineIndex}`}><View style={s.costRow}><TextInput value={line.label} onChangeText={v=>update(itemIndex,group,lineIndex,'label',v)} placeholder="Descrição" style={[s.input,s.costName,!!labelError&&s.inputError]}/><View style={[s.costValue,!!valueError&&s.inputError]}><Text style={s.currency}>R$</Text><TextInput value={line.value} onChangeText={v=>update(itemIndex,group,lineIndex,'value',v)} keyboardType="decimal-pad" style={s.costValueInput}/></View><Pressable onPress={()=>remove(itemIndex,group,lineIndex)} style={s.lineRemove}><Ionicons name="close" size={15} color={theme.danger}/></Pressable></View>{labelError?<Text style={s.errorText}>{labelError}</Text>:valueError?<Text style={s.errorText}>{valueError}</Text>:null}</View>})}<Pressable onPress={()=>add(itemIndex,group)} style={s.miniAdd}><Ionicons name="add" size={13} color={brand.primary}/><Text style={[s.miniAddText,{color:brand.primary}]}>Adicionar</Text></Pressable></View>}
function CalcLine({label,value,gold}:{label:string;value:number;gold?:boolean}){const brand=useTenantBrand();return <View style={s.calcLine}><Text style={s.calcLineLabel}>{label}</Text><Text style={[s.calcLineValue,gold&&s.calcLineGold,gold&&{color:brand.secondary}]}>{money(value)}</Text></View>}
...
const s=StyleSheet.create({
  ...
  stageSummary:{marginTop:16,borderTopWidth:1,borderTopColor:theme.border,paddingTop:13},stageSummaryHead:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',marginBottom:6},stageRow:{minHeight:34,flexDirection:'row',alignItems:'center',gap:8,paddingVertical:5,borderBottomWidth:1,borderBottomColor:'#EEF1EE'},stageIndex:{width:22,height:22,borderRadius:11,backgroundColor:theme.green50,alignItems:'center',justifyContent:'center'},stageIndexText:{fontSize:10,fontWeight:'900',color:theme.green2},stageDescription:{flex:1,fontSize:11,color:theme.ink},stageDuration:{fontSize:10,fontWeight:'800',color:theme.muted},stageEditRow:{flexDirection:'row',alignItems:'flex-start',gap:7,paddingVertical:6,borderBottomWidth:1,borderBottomColor:'#EEF1EE'},stageInput:{height:38,borderWidth:1,borderColor:theme.border,borderRadius:8,paddingHorizontal:9,fontSize:11,color:theme.ink,backgroundColor:theme.white},stageDurationInput:{width:88,height:38,borderWidth:1,borderColor:theme.border,borderRadius:8,paddingHorizontal:8,fontSize:11,color:theme.ink,backgroundColor:theme.white},stageAdd:{alignSelf:'flex-start',flexDirection:'row',alignItems:'center',gap:4,marginTop:8,paddingVertical:5},fixedCard:{borderWidth:1,borderColor:theme.border,borderRadius:13,padding:15},liveCard:{borderWidth:1,borderColor:theme.goldLight,borderRadius:13,padding:15,backgroundColor:theme.goldPale},calcLine:{flexDirection:'row',justifyContent:'space-between',gap:10,paddingVertical:7,borderBottomWidth:1,borderBottomColor:'rgba(201,168,76,.18)'},calcLineLabel:{flex:1,fontSize:11,color:theme.muted},calcLineValue:{fontSize:11,fontWeight:'800',color:theme.ink},calcLineGold:{color:theme.gold},liveTotal:{backgroundColor:theme.green,borderRadius:10,padding:13,marginTop:10,flexDirection:'row',justifyContent:'space-between',alignItems:'center'},liveTotalLabel:{fontSize:11,color:'rgba(255,255,255,.7)'},liveTotalValue:{fontFamily:'serif',fontSize:17,fontWeight:'700',color:theme.goldLight},marginField:{marginTop:11},
