...
export function ProjectsScreen(){
  ...
  const{width}=useWindowDimensions();const compact=width<820;const narrow=width<520;
...
function Column({column,projects,width,statuses,dragging,target,onDragEnter,onDrop,onDragStart,onDragEnd,onMove}:{column:ProjectStatus;projects:ProjectRecord[];width?:number;statuses:ProjectStatus[];dragging:boolean;target:boolean;onDragEnter:()=>void;onDrop:()=>void;onDragStart:(id:string)=>void;onDragEnd:()=>void;onMove:(id:string,status:string)=>Promise<void>}){
  const brand=useTenantBrand();
  const content=<View style={[s.column,{minHeight:0},width?{width}:undefined,dragging&&s.columnDragging,target&&s.columnTarget,target&&{backgroundColor:brand.primarySoft,borderColor:brand.primary}]}><View style={s.columnHead}><View style={[s.dot,{backgroundColor:column.color}]}/><Text style={s.columnTitle}>{column.name}</Text><View style={s.count}><Text style={s.countText}>{projects.length}</Text></View></View><View style={[s.cards,{flex:1}]}>{projects.map(project=><ProjectCard key={project.id} project={project} color={column.color} statuses={statuses} onDragStart={()=>onDragStart(project.id)} onDragEnd={onDragEnd} onMove={status=>onMove(project.id,status)}/>)}{!projects.length&&<View style={[s.emptyColumn,{height:undefined,minHeight:76,flex:1},target&&s.emptyColumnTarget,target&&{backgroundColor:brand.primarySoft,borderColor:brand.primary}]}><Ionicons name={target?'arrow-down-circle-outline':'layers-outline'} size={23} color={target?brand.primary:theme.border}/><Text style={[s.emptyText,target&&{color:brand.primary,fontWeight:'800'}]}>{target?'Solte o projeto aqui':'Nenhum projeto'}</Text></View>}</View></View>;
...
function ProjectCard({project,color,statuses,onDragStart,onDragEnd,onMove}:{project:ProjectRecord;color:string;statuses:ProjectStatus[];onDragStart:()=>void;onDragEnd:()=>void;onMove:(status:string)=>Promise<void>}){
  ...
  const card=<Pressable onPress={()=>{if(didDrag.current)return;router.push(`/projects/${project.id}` as Href)}} onLongPress={()=>Platform.OS!=='web'&&setMoveOpen(true)} style={[s.card,{borderLeftColor:color},overdue>0&&s.overdueCard,Platform.OS==='web'&&({cursor:'grab'} as any)]}>
      <View style={s.cardTop}><View style={s.dragHandle}><Ionicons name="reorder-three-outline" size={18} color={theme.muted}/></View><Text numberOfLines={2} style={s.name}>{project.name}</Text><Pressable accessibilityLabel="Mover projeto" hitSlop={8} onPress={event=>{event.stopPropagation();setMoveOpen(true)}} style={[s.moveButton,{borderColor:brand.primary}]}><Ionicons name="swap-horizontal-outline" size={16} color={brand.primary}/></Pressable><Ionicons name="chevron-forward" size={17} color={theme.muted}/></View>
      <Text style={s.client}>{project.client?.name}</Text><Text style={[s.assignee,{color:brand.primary}]}>Responsável: {project.assignee?.name||'Não definido'}</Text>{(overdue>0||high>0)&&<View style={s.alerts}>{overdue>0&&<Text style={s.dangerTag}>{overdue} atrasada(s)</Text>}{high>0&&<Text style={s.highTag}>{high} alta prioridade</Text>}</View>}<View style={s.metaRow}><Text style={s.meta}>{project.startDate?formatDateBR(project.startDate):'Data a definir'}</Text>{project.quote&&<Text style={[s.quote,{color:brand.secondary}]}>{project.quote.number}</Text>}</View><View style={s.progressHead}><Text style={s.progressLabel}>Progresso</Text><Text style={[s.progressValue,{color:brand.primary}]}>{project.progress}%</Text></View><View style={s.track}><View style={[s.bar,{width:`${project.progress}%`,backgroundColor:color}]}/></View>{project.quote&&<View style={s.footer}><Text style={s.footerLabel}>Valor do projeto</Text><Text style={s.value}>{money(project.quote.finalTotalCents||project.quote.totalCents)}</Text></View>}
    </Pressable>;
  return <>
...
function MoveStatusModal({visible,current,statuses,onClose,onSelect}:{visible:boolean;current:string;statuses:ProjectStatus[];onClose:()=>void;onSelect:(status:string)=>void}){const brand=useTenantBrand();return <Modal visible={visible} transparent animationType="fade" presentationStyle="overFullScreen" onRequestClose={onClose}><Pressable style={s.backdrop} onPress={onClose}><Pressable style={s.moveSheet} onPress={()=>{}}><Text style={s.moveTitle}>Mover projeto para</Text><Text style={s.moveSubtitle}>Escolha a nova etapa do fluxo.</Text>{statuses.map(status=><Pressable key={status.key} disabled={status.key===current} onPress={()=>onSelect(status.key)} style={({pressed})=>[s.statusOption,status.key===current&&s.statusOptionCurrent,status.key===current&&{backgroundColor:brand.primarySoft},pressed&&s.pressed]}><View style={[s.statusDot,{backgroundColor:status.color}]}/><Text style={[s.statusText,status.key===current&&s.statusTextCurrent,status.key===current&&{color:brand.primary}]}>{status.name}</Text>{status.key===current?<Ionicons name="checkmark-circle" size={18} color={brand.primary}/>:<Ionicons name="chevron-forward" size={16} color={theme.muted}/>}</Pressable>)}<Pressable onPress={onClose} style={s.cancelMove}><Text style={s.cancelMoveText}>Cancelar</Text></Pressable></Pressable></Pressable></Modal>}
function Summary({icon,label,value,danger}:{icon:IoniconName;label:string;value:string;danger?:boolean}){const brand=useTenantBrand();return <View style={[s.summary,danger&&s.summaryDanger]}><View style={[s.summaryIcon,!danger&&{backgroundColor:brand.primarySoft}]}><Ionicons name={icon} size={17} color={danger?theme.danger:brand.primary}/></View><View><Text style={s.summaryLabel}>{label}</Text><Text style={[s.summaryValue,danger&&{color:theme.danger}]}>{value}</Text></View></View>}
...
const s=StyleSheet.create({page:{width:'100%',maxWidth:1440,alignSelf:'center'},pageNarrow:{paddingHorizontal:0},summaryRow:{flexDirection:'row',flexWrap:'wrap',gap:10,marginBottom:14},summaryRowNarrow:{flexDirection:'column'},summary:{minWidth:190,flex:1,backgroundColor:theme.white,borderWidth:1,borderColor:theme.border,borderRadius:12,padding:13,flexDirection:'row',alignItems:'center',gap:10},summaryDanger:{borderColor:'#E7C7C1'},summaryIcon:{width:34,height:34,borderRadius:9,backgroundColor:theme.green50,alignItems:'center',justifyContent:'center'},summaryLabel:{fontSize:11,color:theme.muted},summaryValue:{fontFamily:'serif',fontSize:18,fontWeight:'800',color:theme.ink},filters:{flexDirection:'row',gap:6,flexWrap:'wrap'},filtersNarrow:{width:'100%',gap:7},toolbarActions:{flexDirection:'row',alignItems:'flex-end',gap:8,flexWrap:'wrap'},toolbarActionsNarrow:{width:'100%',flexDirection:'column',alignItems:'stretch',gap:10},assigneeFilter:{minWidth:190},assigneeFilterNarrow:{width:'100%',minWidth:0},dragHint:{minHeight:34,marginTop:8,paddingHorizontal:10,flexDirection:'row',alignItems:'center',gap:7,backgroundColor:theme.green50,borderRadius:9,alignSelf:'flex-start',maxWidth:'100%'},dragHintText:{fontSize:11,color:theme.green2,fontWeight:'700'},board:{flexGrow:1,alignItems:'flex-start',gap:12,paddingTop:14,paddingBottom:8},boardCompact:{flexDirection:'column'},column:{width:330,minHeight:330,backgroundColor:'#EFF3F0',borderRadius:14,padding:11,borderWidth:1,borderColor:'transparent'},columnDragging:{borderStyle:'dashed',borderColor:'#C7D7CD'},columnTarget:{backgroundColor:'#E8F1EB',borderColor:theme.green2},columnHead:{height:38,flexDirection:'row',alignItems:'center',gap:7,paddingHorizontal:3},dot:{width:7,height:7,borderRadius:4},columnTitle:{flex:1,fontSize:12,fontWeight:'900',color:theme.ink,textTransform:'uppercase'},count:{minWidth:23,height:23,borderRadius:12,backgroundColor:theme.white,alignItems:'center',justifyContent:'center'},countText:{fontSize:11,fontWeight:'800',color:theme.muted},cards:{gap:9},card:{backgroundColor:theme.white,borderWidth:1,borderLeftWidth:3,borderColor:theme.border,borderRadius:11,padding:13},overdueCard:{borderColor:'#E6B8AF'},cardTop:{flexDirection:'row',alignItems:'flex-start',gap:6},dragHandle:{width:20,height:20,alignItems:'center',justifyContent:'center'},moveButton:{width:28,height:28,borderRadius:8,borderWidth:1,borderColor:theme.border,alignItems:'center',justifyContent:'center',marginTop:-4},name:{flex:1,fontSize:13,fontWeight:'800',lineHeight:17,color:theme.ink},client:{fontSize:11,color:theme.muted,marginTop:4,marginLeft:26},assignee:{fontSize:10,color:theme.green2,fontWeight:'800',marginTop:4,marginLeft:26},alerts:{flexDirection:'row',flexWrap:'wrap',gap:6,marginTop:8},dangerTag:{fontSize:10,fontWeight:'900',color:theme.danger,backgroundColor:'#FBE9E6',paddingHorizontal:7,paddingVertical:4,borderRadius:7},highTag:{fontSize:10,fontWeight:'900',color:'#8A6D22',backgroundColor:theme.goldPale,paddingHorizontal:7,paddingVertical:4,borderRadius:7},metaRow:{flexDirection:'row',flexWrap:'wrap',justifyContent:'space-between',gap:6,marginTop:11},meta:{fontSize:11,color:theme.muted},quote:{fontSize:11,fontWeight:'800',color:theme.gold},progressHead:{flexDirection:'row',justifyContent:'space-between',marginTop:11},progressLabel:{fontSize:11,color:theme.muted},progressValue:{fontSize:11,fontWeight:'800',color:theme.green2},track:{height:5,backgroundColor:theme.border,borderRadius:3,marginTop:5},bar:{height:5,borderRadius:3},footer:{borderTopWidth:1,borderTopColor:theme.border,marginTop:11,paddingTop:9,flexDirection:'row',flexWrap:'wrap',gap:6,justifyContent:'space-between'},footerLabel:{fontSize:11,color:theme.muted},value:{fontSize:11,fontWeight:'800',color:theme.ink},emptyColumn:{height:130,alignItems:'center',justifyContent:'center',gap:7},emptyColumnTarget:{backgroundColor:'#F7FBF8',borderWidth:1,borderStyle:'dashed',borderColor:theme.green2,borderRadius:10},emptyText:{fontSize:11,color:theme.muted},backdrop:{flex:1,backgroundColor:'rgba(11,29,21,.36)',alignItems:'center',justifyContent:'center',padding:18},moveSheet:{width:'100%',maxWidth:360,borderRadius:17,backgroundColor:theme.white,borderWidth:1,borderColor:theme.border,padding:14,shadowColor:'#000',shadowOpacity:.18,shadowRadius:24,elevation:16},moveTitle:{fontFamily:'serif',fontSize:18,fontWeight:'800',color:theme.ink},moveSubtitle:{fontSize:11,color:theme.muted,marginTop:3,marginBottom:10},statusOption:{minHeight:48,borderRadius:10,paddingHorizontal:10,flexDirection:'row',alignItems:'center',gap:9},statusOptionCurrent:{backgroundColor:theme.green50},statusDot:{width:9,height:9,borderRadius:5},statusText:{flex:1,fontSize:12,fontWeight:'800',color:theme.ink},statusTextCurrent:{color:theme.green2},cancelMove:{height:43,borderTopWidth:1,borderTopColor:theme.border,marginTop:6,alignItems:'center',justifyContent:'center'},cancelMoveText:{fontSize:11,fontWeight:'800',color:theme.muted},pressed:{opacity:.72}});
