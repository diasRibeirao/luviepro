...
import {SortMenu} from '../../../components/ListControls';
import {formatDateBR} from '../../../components/DateField';
import {useFeedback} from '../../../components/Feedback';
import {Text} from '../../../i18n';
import {projectsApi} from '../api/projects.api';
import type {ProjectRecord,ProjectStatus,ProjectTask} from '../types/project.types';
...
export function ProjectsScreen(){
  const[list,setList]=useState<ProjectRecord[]>([]),[statuses,setStatuses]=useState<ProjectStatus[]>(fallback),[loading,setLoading]=useState(true),[loadError,setLoadError]=useState(''),[query,setQuery]=useState(''),[filter,setFilter]=useState('Todos'),[sort,setSort]=useState('priority'),[boardWidth,setBoardWidth]=useState(0),[draggedId,setDraggedId]=useState<string>(),[dropStatus,setDropStatus]=useState<string>();
  const{notify}=useFeedback();
  const{width}=useWindowDimensions();const compact=width<820;
  const load=useCallback(()=>{setLoading(true);setLoadError('');Promise.all([projectsApi.list(),projectsApi.listStatuses()]).then(([projects,configured])=>{setList(projects);setStatuses(configured?.length?configured:fallback)}).catch((e:unknown)=>setLoadError(errorMessage(e))).finally(()=>setLoading(false))},[]);useFocusEffect(load);
  const filtered=useMemo(()=>{const items=list.filter(project=>{const term=query.trim().toLowerCase();const matches=!term||`${project.name} ${project.client?.name??''} ${project.quote?.number??''}`.toLowerCase().includes(term);const overdue=project.tasks?.some(isOverdue);const high=project.tasks?.some(t=>t.status!=='completed'&&t.priority==='high');return matches&&(filter==='Todos'||filter==='Com atraso'&&overdue||filter==='Alta prioridade'&&high)});return [...items].sort((a,b)=>{const critical=(p:ProjectRecord)=>(p.tasks?.filter(isOverdue).length??0)*100+(p.tasks?.filter(t=>t.status!=='completed'&&t.priority==='high').length??0);if(sort==='name')return String(a.name).localeCompare(String(b.name),'pt-BR');if(sort==='progress-desc')return b.progress-a.progress;if(sort==='progress-asc')return a.progress-b.progress;return critical(b)-critical(a)})},[list,query,filter,sort]);
