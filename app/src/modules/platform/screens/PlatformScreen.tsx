...
export default function Platform(){
 ...
 {tab==='overview'?<Overview data={data} companies={companies} users={users} payments={payments} onTab={selectTab}/>:<>
  ...
  {tab!=='plans'?<PlatformFilterBar>
   ...
   <PlatformFilterGroup label="Plano" value={planFilter} onChange={value=>{setPlanFilter(value);setPage(1)}} values={[['all','Todos'],...plans.map(plan=>[plan.plan,plan.name??plan.plan.toUpperCase()] as const)]}/>
