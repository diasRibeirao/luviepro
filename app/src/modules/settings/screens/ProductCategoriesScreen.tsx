...
export default function ProductCategoriesScreen(){
 ...
 const {width}=useWindowDimensions();const compact=width<720;const narrow=width<520; const{notify,confirm}=useFeedback();
 ...
 return <AppShell title="Categorias de produtos" subtitle="Cadastros usados na classificação dos produtos." action={<HeaderAction label="Nova categoria" onPress={create}/>}> 
  ...
  <FormModal visible={open} title={form.id?'Editar categoria':'Nova categoria'} subtitle="Informe os dados da categoria de produto." onClose={()=>!busy&&setOpen(false)} footer={<><Pressable style={s.cancel} onPress={()=>setOpen(false)}><Text style={s.cancelText}>Cancelar</Text></Pressable><AsyncButton busy={busy} label={form.id?'Salvar categoria':'Cadastrar categoria'} onPress={save}/></>}> 
   ...
   <Pressable onPress={()=>setForm(f=>({...f,active:!f.active}))} style={[s.activeChoice,form.active&&s.activeChoiceOn,form.active&&{backgroundColor:brand.primarySoft,borderColor:brand.primary}]}><Ionicons name={form.active?'checkmark-circle':'ellipse-outline'} size={18} color={form.active?brand.primary:theme.muted}/><Text style={s.activeChoiceText}>{form.active?'Categoria ativa':'Categoria inativa'}</Text></Pressable>
  </FormModal>
 </AppShell>;
}
function Summary({label,value,icon}:{label:string;value:string;icon:any}){const brand=useTenantBrand();return <View style={s.summary}><View style={[s.summaryIcon,{backgroundColor:brand.primarySoft}]}><Ionicons name={icon} size={17} color={brand.primary}/></View><View><Text style={s.summaryLabel}>{label}</Text><Text style={s.summaryValue}>{value}</Text></View></View>}
...
function Filter({label,active,onPress}:{label:string;active:boolean;onPress:()=>void}){const brand=useTenantBrand();return <Pressable onPress={onPress} style={[s.filter,active&&s.filterOn,active&&{backgroundColor:brand.primary}]}><Text style={[s.filterText,active&&s.filterTextOn,active&&{color:brand.primaryForeground}]}>{label}</Text></Pressable>}
const s=StyleSheet.create({back:{alignSelf:'flex-start',flexDirection:'row',alignItems:'center',gap:6,marginBottom:14,paddingVertical:4},backText:{fontSize:12,fontWeight:'800',color:theme.green2},summaryRow:{flexDirection:'row',gap:12,marginBottom:18},summaryCompact:{flexWrap:'wrap'},summaryNarrow:{flexDirection:'column'},summary:{minWidth:190,flex:1,backgroundColor:theme.white,borderWidth:1,borderColor:theme.border,borderRadius:14,padding:15,flexDirection:'row',alignItems:'center',gap:11},summaryIcon:{width:36,height:36,borderRadius:10,backgroundColor:theme.green50,alignItems:'center',justifyContent:'center'},summaryLabel:{fontSize:11,color:theme.muted},summaryValue:{fontFamily:'serif',fontSize:20,fontWeight:'800',color:theme.ink,marginTop:2},panel:{backgroundColor:theme.white,borderWidth:1,borderColor:theme.border,borderRadius:16,overflow:'hidden'},toolbar:{padding:14,borderBottomWidth:1,borderBottomColor:theme.border,flexDirection:'row',alignItems:'center',gap:12},toolbarCompact:{flexDirection:'column',alignItems:'stretch'},toolbarNarrow:{padding:11},search:{flex:1,minWidth:220},filters:{flexDirection:'row',gap:6,flexWrap:'wrap'},filter:{height:36,borderRadius:9,paddingHorizontal:12,alignItems:'center',justifyContent:'center'},filterOn:{backgroundColor:theme.green},filterText:{fontSize:11,fontWeight:'900',color:theme.muted},filterTextOn:{color:theme.white},row:{minHeight:72,paddingHorizontal:18,paddingVertical:12,flexDirection:'row',alignItems:'center',gap:10,borderTopWidth:1,borderTopColor:theme.border},rowNarrow:{flexWrap:'wrap',alignItems:'flex-start',paddingHorizontal:12},first:{borderTopWidth:0},rowInfo:{flex:1,minWidth:0},rowTitle:{fontSize:13,fontWeight:'900',color:theme.ink},rowMeta:{fontSize:11,color:theme.muted,marginTop:4},status:{minWidth:78,height:30,borderRadius:999,borderWidth:1,borderColor:theme.border,paddingHorizontal:9,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:5},statusOn:{backgroundColor:theme.successSoft,borderColor:'#CFE7D8'},statusOff:{backgroundColor:theme.cream},statusText:{fontSize:10,fontWeight:'900',color:theme.muted},statusTextOn:{color:theme.success},iconButton:{width:36,height:36,borderRadius:9,borderWidth:1,borderColor:theme.border,alignItems:'center',justifyContent:'center',backgroundColor:theme.white},empty:{padding:38,alignItems:'center',gap:7},emptyTitle:{fontSize:12,fontWeight:'800',color:theme.muted},pagination:{minHeight:54,paddingHorizontal:14,borderTopWidth:1,borderTopColor:theme.border,flexDirection:'row',alignItems:'center',justifyContent:'space-between',gap:10},paginationNarrow:{flexDirection:'column',alignItems:'stretch',gap:8,paddingVertical:12},pageInfo:{fontSize:11,color:theme.muted},pageButtons:{flexDirection:'row',alignItems:'center',gap:8},pageButton:{width:34,height:34,borderWidth:1,borderColor:theme.border,borderRadius:8,alignItems:'center',justifyContent:'center'},pageNumber:{fontSize:11,fontWeight:'800',color:theme.ink},disabled:{opacity:.35},cancel:{height:40,paddingHorizontal:15,justifyContent:'center'},cancelText:{fontSize:12,fontWeight:'800',color:theme.muted},activeChoice:{minHeight:42,borderWidth:1,borderColor:theme.border,borderRadius:10,paddingHorizontal:12,flexDirection:'row',alignItems:'center',gap:7},activeChoiceOn:{backgroundColor:theme.green50,borderColor:'#C9D8CE'},activeChoiceText:{fontSize:12,fontWeight:'800',color:theme.ink}});
