...
export function UsersPanel({limit,plan}:{limit:number;plan:string}){
  ...
  return <Section title="Usuários e acessos" subtitle="Convide pessoas para o sistema sem compartilhar senhas. Equipe operacional e usuários licenciados continuam sendo conceitos separados.">
    <View style={s.accessSummary}>
      ...
      {(starter||full)&&<Pressable onPress={()=>router.push('/plans')} style={[s.upgradeButton,{borderColor:brand.primary}]}><Text style={[s.upgradeButtonText,{color:brand.primary}]}>Ver planos</Text></Pressable>}
    </View>
    {!starter&&<>
      ...
      <View style={s.profileGrid}>{['admin','commercial','operational','finance'].map(value=><Pressable key={value} onPress={()=>{setRole(value);setCustomProfileId(undefined)}} style={[s.profileCard,role===value&&s.profileCardOn,role===value&&{borderColor:brand.primary,backgroundColor:brand.primarySoft}]}><View style={s.profileHead}><Ionicons name={(value==='admin'?'shield-checkmark-outline':value==='commercial'?'briefcase-outline':value==='operational'?'construct-outline':'cash-outline') as IoniconName} size={18} color={role===value?brand.primary:theme.muted}/><Text style={[s.profileName,role===value&&s.profileNameOn,role===value&&{color:brand.primary}]}>{roleLabels[value]}</Text></View><Text style={s.profileHelp}>{roleHelp[value]}</Text></Pressable>)}</View>
      {plan==='business'&&profiles.filter(p=>p.active).length>0&&<><Text style={[s.label,{marginTop:14}]}>PERFIS PERSONALIZADOS</Text><View style={s.profileGrid}>{profiles.filter(p=>p.active).map(profile=><Pressable key={profile.id} onPress={()=>{setCustomProfileId(profile.id);setRole('admin')}} style={[s.profileCard,customProfileId===profile.id&&s.profileCardOn,customProfileId===profile.id&&{borderColor:brand.primary,backgroundColor:brand.primarySoft}]}><View style={s.profileHead}><Ionicons name="key-outline" size={18} color={customProfileId===profile.id?brand.primary:theme.muted}/><Text style={[s.profileName,customProfileId===profile.id&&s.profileNameOn,customProfileId===profile.id&&{color:brand.primary}]}>{profile.name}</Text></View><Text style={s.profileHelp}>{profile.description||`${Array.isArray(profile.permissions)?profile.permissions.length:0} permissões configuradas`}</Text></Pressable>)}</View></>}
      <Pressable disabled={busy||full||!name.trim()||!isValidEmail(email)} onPress={add} style={[s.planButton,{backgroundColor:brand.primary},(busy||full||!name.trim()||!isValidEmail(email))&&{opacity:.45}]}><Ionicons name="mail-unread-outline" size={16} color={brand.primaryForeground}/><Text style={[s.planButtonText,{color:brand.primaryForeground}]}>{busy?'Enviando...':'Enviar convite'}</Text></Pressable>
      {lastInvite?.inviteUrl&&<View style={[s.inviteLinkCard,{backgroundColor:brand.primarySoft}]}><Ionicons name="link-outline" size={18} color={brand.primary}/><View style={{flex:1}}><Text style={s.inviteLinkTitle}>{lastInvite.delivery?.sent?'Convite enviado por e-mail':'Link para compartilhamento manual'}</Text><Text numberOfLines={1} style={s.inviteLinkText}>{lastInvite.inviteUrl}</Text></View><Pressable onPress={()=>copyInvite(lastInvite.inviteUrl)} style={s.smallButton}><Text style={[s.smallButtonText,{color:brand.primary}]}>Copiar link</Text></Pressable></View>}
    </>}
    {invites.length>0&&<><Text style={[s.blockTitle,{marginTop:22}]}>Convites</Text><View>{invites.map(invite=><View key={invite.id} style={s.userRow}><View style={[s.userAvatar,{backgroundColor:brand.primarySoft}]}><Ionicons name="mail-outline" size={16} color={brand.primary}/></View><View style={{flex:1}}><Text style={s.userName}>{invite.name}</Text><Text style={s.userMeta}>{invite.email} · {invite.customProfile?.name??roleLabels[invite.role]??invite.role} · expira {new Date(invite.expiresAt).toLocaleString('pt-BR')}</Text></View><View style={[s.statusBadge,invite.status==='pending'?s.pendingBadge:s.inactiveBadge]}><Text style={s.statusText}>{invite.status==='pending'?'Pendente':'Expirado'}</Text></View><Pressable disabled={busy} onPress={()=>resend(invite)} style={s.smallButton}><Text style={[s.smallButtonText,{color:brand.primary}]}>Reenviar</Text></Pressable><Pressable disabled={busy} onPress={()=>cancelInvite(invite)} style={s.smallButton}><Text style={[s.smallButtonText,{color:theme.danger}]}>Cancelar</Text></Pressable></View>)}</View></>}
    <Text style={[s.blockTitle,{marginTop:22}]}>Pessoas com acesso</Text>
    <View>{loading||error?<AsyncState loading={loading} error={error} onRetry={load}/>:users.map(user=><View key={user.id} style={s.userRow}><View style={[s.userAvatar,{backgroundColor:brand.primarySoft}]}><Text style={[s.userAvatarText,{color:brand.primary}]}>{user.name?.slice(0,2).toUpperCase()}</Text></View><View style={{flex:1}}><Text style={s.userName}>{user.name}</Text><Text style={s.userMeta}>{user.email} · {user.customProfile?.name??roleLabels[user.role]??user.role}</Text></View><View style={[s.statusBadge,user.active?s.activeBadge:s.inactiveBadge]}><Text style={s.statusText}>{user.active?'Ativo':'Inativo'}</Text></View>{user.role!=='owner'&&<Pressable onPress={()=>toggle(user)} style={s.smallButton}><Text style={[s.smallButtonText,{color:brand.primary}]}>{user.active?'Desativar':'Ativar'}</Text></Pressable>}</View>)}</View>
    {plan==='business'&&<View style={[s.businessNote,{backgroundColor:brand.secondarySoft,borderColor:brand.secondary}]}><Ionicons name="diamond-outline" size={17} color={brand.secondary}/><View style={{flex:1}}><Text style={s.businessTitle}>Business · controle avançado</Text><Text style={s.businessText}>Auditoria, perfis personalizados e permissões granulares estão habilitados neste plano.</Text></View></View>}
  </Section>
}
