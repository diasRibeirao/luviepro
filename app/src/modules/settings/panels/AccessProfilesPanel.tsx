...
export function AccessProfilesPanel(){
  ...
  return <Section title="Perfis e permissões" subtitle="Crie perfis personalizados para o plano Business e escolha exatamente quais áreas cada pessoa pode acessar.">
    <View style={[s.businessNote,{backgroundColor:brand.secondarySoft,borderColor:brand.secondary}]}><Ionicons name="shield-checkmark-outline" size={18} color={brand.secondary}/><View style={{flex:1}}><Text style={s.businessTitle}>Permissões aplicadas no backend</Text><Text style={s.businessText}>O menu é ajustado ao perfil e as operações protegidas são validadas novamente pela API.</Text></View></View>
    ...
    <View style={s.permissionGroups}>{permissionGroups.map(([group,items])=><View key={group} style={s.permissionGroup}><Text style={s.permissionGroupTitle}>{group}</Text>{items.map(([code,label])=><Pressable key={code} onPress={()=>togglePermission(code)} style={s.permissionRow}><View style={[s.permissionCheck,permissions.includes(code)&&s.permissionCheckOn,permissions.includes(code)&&{backgroundColor:brand.primary,borderColor:brand.primary}]}>{permissions.includes(code)&&<Ionicons name="checkmark" size={13} color={brand.primaryForeground}/>}</View><Text style={s.permissionLabel}>{label}</Text><Text style={s.permissionCode}>{code}</Text></Pressable>)}</View>)}</View>
    {formErrors.permissions?<Text style={s.fieldError}>{formErrors.permissions}</Text>:null}
    <View style={s.profileActions}><Pressable onPress={reset} style={s.smallButton}><Text style={[s.smallButtonText,{color:brand.primary}]}>Limpar</Text></Pressable><Pressable disabled={busy} onPress={save} style={[s.planButton,{marginTop:0,backgroundColor:brand.primary},busy&&{opacity:.45}]}><Text style={[s.planButtonText,{color:brand.primaryForeground}]}>{busy?'Salvando...':editing?'Salvar perfil':'Criar perfil'}</Text></Pressable></View>
    <Text style={[s.blockTitle,{marginTop:22}]}>Perfis cadastrados</Text>
    {loading||error?<AsyncState loading={loading} error={error} onRetry={load}/>:list.length===0?<AsyncState empty emptyTitle="Nenhum perfil personalizado" emptyMessage="Crie o primeiro perfil para controlar acessos por área."/>:list.map(profile=><View key={profile.id} style={s.userRow}><View style={[s.userAvatar,{backgroundColor:brand.primarySoft}]}><Ionicons name="key-outline" size={16} color={brand.primary}/></View><View style={{flex:1}}><Text style={s.userName}>{profile.name}</Text><Text style={s.userMeta}>{profile.description||'Sem descrição'} · {Array.isArray(profile.permissions)?profile.permissions.length:0} permissões</Text></View><View style={[s.statusBadge,profile.active?s.activeBadge:s.inactiveBadge]}><Text style={s.statusText}>{profile.active?'Ativo':'Inativo'}</Text></View><Pressable onPress={()=>edit(profile)} style={s.smallButton}><Text style={[s.smallButtonText,{color:brand.primary}]}>Editar</Text></Pressable><Pressable onPress={()=>toggleActive(profile)} style={s.smallButton}><Text style={[s.smallButtonText,{color:brand.primary}]}>{profile.active?'Inativar':'Ativar'}</Text></Pressable></View>)}
  </Section>
}
