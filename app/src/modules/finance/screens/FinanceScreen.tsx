...
export function FinanceScreen(){
  ...
  const load=async(months=reportMonths)=>{
    setLoading(true);
    try{
      const [s,e,o,c,m,r]=await Promise.all([
        financeApi.summary(),financeApi.entries(),financeApi.obligations(),financeApi.categories(),financeApi.paymentMethods(),financeApi.report(months),
      ]);
      setSummary(s);setEntries(e);setObligations(o);setCategories(c);setPaymentMethods(m);setReport(r);
      if(m.length&&!m.some(x=>x.code===method))setMethod((m.find(x=>x.code==='pix')??m[0]).code);
    }catch(err:any){
  ...
  const preferredMethod=()=>paymentMethods.find(x=>x.code==='pix')?.code??paymentMethods[0]?.code??'';
  const resetForm=()=>{setType('expense');setDescription('');setCounterparty('');setAmount('');setDueAt('');setEntryStatus('pending');setMethod(preferredMethod());setNotes('');setCategoryId('');setErrors({})};
  ...
  const openPay=(row:FinanceObligation)=>{setSelected(row);setMethod(preferredMethod());setNotes('');setPayOpen(true)};
  ...
  return <AppShell title="Financeiro" subtitle="Previsto × realizado, contas abertas, fluxo de caixa e relatórios">
    ...
    <FormModal visible={payOpen} title="Dar baixa no lançamento" subtitle={selected?`${selected.description} · ${money(selected.amountCents)}`:undefined} onClose={()=>setPayOpen(false)}><Text style={s.label}>FORMA DE PAGAMENTO</Text><View style={s.choices}>{paymentMethods.map(m=><Pressable key={m.id} onPress={()=>setMethod(m.code)} style={[s.choice,method===m.code&&s.choiceActive]}><Text style={[s.choiceText,method===m.code&&s.choiceTextActive]}>{m.name}</Text></Pressable>)}</View><Field label="Observações" value={notes} onChangeText={setNotes}/><Pressable disabled={saving} style={s.primary} onPress={()=>void payManual()}><Text style={s.primaryText}>Confirmar baixa</Text></Pressable></FormModal>
  </AppShell>;
}
