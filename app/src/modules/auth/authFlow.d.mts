...
export type AuthRoute='/'|'/home'|'/platform'|'/plans';
...
export function postLoginRoute(session:Pick<AuthSessionResponse,'platform'|'user'|'tenant'>):AuthRoute;
export function isPublicAuthRoute(path:string):boolean;
export function isBillingRestrictedSession(session:unknown):boolean;
export function authGuardRedirect(authenticated:boolean,path:string,platform?:boolean,billingRestricted?:boolean):AuthRoute|undefined;
