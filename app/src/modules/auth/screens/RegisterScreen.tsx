...
import { PlanLimit } from '../../../contracts';
...
const periodMonths:Record<BillingPeriod,number>={monthly:1,quarterly:3,semiannual:6,annual:12};
const featureList=(plan:PlanLimit)=>{
  const limit=(value:number,label:string)=>value<0?`${label} ilimitados`:`Até ${value} ${label.toLowerCase()}`;
  const features=[
    limit(plan.maxClients,'Clientes'),
    plan.maxQuotesPerMonth<0?'Orçamentos ilimitados':`Até ${plan.maxQuotesPerMonth} orçamentos/mês`,
    plan.maxUsers<0?'Usuários ilimitados':plan.maxUsers===1?'1 usuário':`Até ${plan.maxUsers} usuários`,
    plan.logoPdf!==false?'PDF com logo':null,
    plan.customPdf?'PDF personalizado':null,
    plan.projectManagement&&plan.projectManagement!=='basic'?'Projetos + etapas':'Dashboard básico',
    plan.advancedReports?'Relatórios avançados':null,
    plan.auditAccess?'Auditoria de atividades':null,
    plan.exportData?'Exportação de dados':null,
  ];
  return features.filter((item):item is string=>Boolean(item));
};
const registerPlan=(plan:PlanLimit,index:number,total:number):RegisterPlan=>{
  const totals:Record<BillingPeriod,string>={
    monthly:money(plan.monthlyPriceCents),
    quarterly:money(plan.quarterlyPriceCents),
    semiannual:money(plan.semiannualPriceCents),
    annual:money(plan.annualPriceCents),
  };
  const prices:Record<BillingPeriod,string>={
    monthly:money(plan.monthlyPriceCents),
    quarterly:money(Math.round(plan.quarterlyPriceCents/3)),
    semiannual:money(Math.round(plan.semiannualPriceCents/6)),
    annual:money(Math.round(plan.annualPriceCents/12)),
  };
...
export default function Register(){
  ...
  async function submit(){
    if(!plan){setError('Nenhum plano ativo está disponível para cadastro no momento.');return}
    const next:Record<string,string>={};if(!company.trim())next.company='Informe o nome da empresa.';if(!name.trim())next.name='Informe seu nome.';if(!email.trim())next.email='Informe seu e-mail.';else if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))next.email='Informe um e-mail válido.';if(password.length<8)next.password='A senha deve ter pelo menos 8 caracteres.';setFieldErrors(next);if(Object.keys(next).length)return;
    try{
      ...
      if(duplicateEmail){
        ...
        setFieldErrors(current=>({...current,email:'Este e-mail já possui uma conta no LuviePro.'}));
        setError('Para adicionar outro usuário à mesma empresa, entre na conta existente e use o gerenciamento de acessos.');
  ...
  return <SafeAreaView style={s.page}>
    ...
    <ScrollView style={s.formSide} contentContainerStyle={[s.formContent,!wide&&{paddingTop:110}]}>
      ...
      {plansLoading?<View style={s.catalogState}><ActivityIndicator color={theme.green2}/><Text style={s.catalogStateText}>Carregando planos disponíveis...</Text></View>:plans.length===0?<View style={s.catalogState}><Ionicons name="information-circle-outline" size={20} color={theme.gold}/><Text style={s.catalogStateText}>Nenhum plano está disponível para cadastro no momento.</Text></View>:<View style={[s.planGrid,!cardsWide&&s.planGridCompact]}>
        {plans.map(item=>{
      ...
      {selectedPlan?<View style={s.selectionSummary}><Ionicons name="shield-checkmark-outline" size={18} color={theme.green2}/><Text style={s.selectionSummaryText}>Você escolheu <Text style={s.selectionSummaryStrong}>{selectedPlan.name}</Text> · {selectedPeriod[1]} · {selectedPlan.totals[period]} por período.</Text></View>:null}
