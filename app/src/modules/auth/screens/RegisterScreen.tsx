...
export default function Register(){
  ...
  async function submit(){
    ...
    const next:Record<string,string>={};if(!company.trim())next.company='Informe o nome da empresa.';if(!name.trim())next.name='Informe seu nome.';if(!email.trim())next.email='Informe seu e-mail.';else if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))next.email='Informe um e-mail válido.';if(password.length<6)next.password='A senha deve ter pelo menos 6 caracteres.';setFieldErrors(next);if(Object.keys(next).length)return;
    try{
  ...
  return <SafeAreaView style={s.page}>
    ...
    <ScrollView style={s.formSide} contentContainerStyle={[s.formContent,!wide&&{paddingTop:110}]}>
      ...
      <View style={s.fields}><Field label="NOME DA EMPRESA" required error={fieldErrors.company} value={company} change={v=>{setCompany(v);setFieldErrors(e=>({...e,company:''}))}}/><Field label="SEU NOME" required error={fieldErrors.name} value={name} change={v=>{setName(v);setFieldErrors(e=>({...e,name:''}))}}/><Field label="TELEFONE / WHATSAPP" value={phone} change={setPhone}/><Field label="E-MAIL" required error={fieldErrors.email} value={email} change={v=>{setEmail(v);setFieldErrors(e=>({...e,email:''}))}}/><Field label="SENHA" required error={fieldErrors.password} value={password} change={v=>{setPassword(v);setFieldErrors(e=>({...e,password:''}))}} password/></View>
