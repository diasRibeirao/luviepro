...
export default function Calendar(){
 ...
 return <AppShell title="Agenda" subtitle="Compromissos, projetos e prazos em um único lugar" action={<HeaderAction label="Novo evento" icon="calendar-outline" onPress={openNew}/> }>
  ...
  <View style={[s.calendar,compact&&s.calendarCompact]}><View style={[s.monthHead,compact&&s.monthHeadCompact]}><Pressable onPress={()=>setMonth(new Date(y,m-1,1))}><Ionicons name="chevron-back" size={20} color={theme.green2}/></Pressable><Text style={s.month}>{month.toLocaleDateString(locale,{month:'long',year:'numeric'})}</Text><Pressable onPress={()=>setMonth(new Date(y,m+1,1))}><Ionicons name="chevron-forward" size={20} color={theme.green2}/></Pressable><Pressable onPress={()=>{const d=new Date();setMonth(d);setSelected(d.toISOString().slice(0,10))}} style={s.today}><Text style={s.todayText}>Hoje</Text></Pressable></View><View style={s.week}>{['SEG','TER','QUA','QUI','SEX','SÁB','DOM'].map(x=><Text key={String(x)} style={s.weekText}>{x}</Text>)}</View><View style={s.grid}>{days.map((d,i)=>{const iso=d?`${y}-${String(m+1).padStart(2,'0')}-${String(d).padStart(2,'0')}`:'';const count=events.filter(e=>occursOn(e,iso)).length;return <Pressable key={i} onPress={()=>d&&setSelected(iso)} style={[s.cell,compact&&s.cellCompact,iso===selected&&s.cellOn]}><Text style={s.cellText}>{d||''}</Text>{count>0&&<View style={s.mark}><Text style={s.markText}>{count}</Text></View>}</Pressable>})}</View></View>
  {loading||error?<AsyncState loading={loading} error={error} onRetry={load}/>:<View style={[s.dayPanel,compact&&s.dayPanelCompact]}><Text style={s.dayTitle}>{new Date(selected+'T12:00:00').toLocaleDateString(locale,{weekday:'long',day:'2-digit',month:'long'})}</Text>{dayEvents.length?dayEvents.map(e=><Pressable style={[s.event,compact&&s.eventCompact]} key={e.id} disabled={!e.projectId} onPress={()=>e.projectId&&router.push(`/projects/${e.projectId}` as Href)}><View style={s.eventIcon}><Ionicons name={e.type==='project'?'briefcase-outline':'time-outline'} size={18} color={theme.green2}/></View><View style={{flex:1}}><Text style={s.eventTitle}>{e.title}</Text><Text style={s.eventMeta}>{e.allDay?`${isoDate(e.startAt)===isoDate(e.endAt)||!e.endAt?'Dia inteiro':`${new Date(e.startAt).toLocaleDateString(locale,{day:'2-digit',month:'2-digit'})} a ${new Date(e.endAt).toLocaleDateString(locale,{day:'2-digit',month:'2-digit'})}`}`:new Date(e.startAt).toLocaleTimeString(locale,{hour:'2-digit',minute:'2-digit'})}{e.location?` · ${e.location}`:''}{e.projectId?' · Abrir projeto':''}</Text></View><Text style={[s.eventType,compact&&s.eventTypeCompact]}>{types[e.type]||'Evento'}</Text></Pressable>):<Text style={s.empty}>Nenhum compromisso neste dia.</Text>}</View>}
...
const s=StyleSheet.create({stats:{flexDirection:'row',gap:10,marginBottom:14},statsCompact:{flexWrap:'wrap'},stat:{flex:1,minWidth:105,backgroundColor:theme.white,borderWidth:1,borderColor:theme.border,borderRadius:12,padding:14},statLabel:{fontSize:11,color:theme.muted},statValue:{fontFamily:'serif',fontSize:22,fontWeight:'800',color:theme.ink},filters:{flexDirection:'row',gap:8,flexWrap:'wrap',marginBottom:14},filter:{borderWidth:1,borderColor:theme.border,borderRadius:18,paddingHorizontal:13,paddingVertical:8},filterOn:{backgroundColor:theme.green2,borderColor:theme.green2},filterText:{fontSize:11,fontWeight:'800',color:theme.muted},filterTextOn:{color:theme.white},calendar:{backgroundColor:theme.white,borderWidth:1,borderColor:theme.border,borderRadius:14,padding:16},calendarCompact:{padding:8},monthHead:{flexDirection:'row',alignItems:'center',gap:16,marginBottom:15},monthHeadCompact:{gap:8,flexWrap:'wrap'},month:{fontWeight:'900',fontSize:16,color:theme.ink,textTransform:'capitalize'},today:{marginLeft:'auto',borderWidth:1,borderColor:theme.border,borderRadius:8,paddingHorizontal:10,paddingVertical:7},todayText:{fontSize:11,fontWeight:'800',color:theme.green2},week:{flexDirection:'row'},weekText:{width:'14.28%',textAlign:'center',fontSize:10,color:theme.muted,fontWeight:'800',paddingVertical:8},grid:{flexDirection:'row',flexWrap:'wrap'},cell:{width:'14.28%',height:56,borderWidth:1,borderColor:'#edf0ee',padding:7},cellCompact:{height:48,padding:4},cellOn:{backgroundColor:'#eef5f0',borderColor:theme.green2},cellText:{fontSize:12,color:theme.ink},mark:{marginTop:5,width:18,height:16,borderRadius:8,backgroundColor:theme.gold,alignItems:'center'},markText:{fontSize:9,fontWeight:'900'},dayPanel:{backgroundColor:theme.white,borderWidth:1,borderColor:theme.border,borderRadius:14,padding:16,marginTop:14},dayPanelCompact:{padding:12},dayTitle:{fontFamily:'serif',fontSize:19,fontWeight:'800',color:theme.ink,marginBottom:10},event:{flexDirection:'row',alignItems:'center',gap:10,borderTopWidth:1,borderTopColor:theme.border,paddingVertical:12},eventCompact:{flexWrap:'wrap'},eventIcon:{width:34,height:34,borderRadius:9,backgroundColor:theme.green50,alignItems:'center',justifyContent:'center'},eventTitle:{fontWeight:'900',color:theme.ink},eventMeta:{fontSize:11,color:theme.muted,marginTop:3},eventType:{fontSize:11,fontWeight:'800',color:theme.green2},eventTypeCompact:{marginLeft:44},empty:{color:theme.muted},label:{fontSize:11,fontWeight:'900',color:theme.muted,marginTop:8},row:{flexDirection:'row',gap:10},rowCompact:{flexDirection:'column'},cancel:{paddingHorizontal:15,paddingVertical:11},linkedNotice:{minHeight:42,borderRadius:10,backgroundColor:theme.green50,paddingHorizontal:12,flexDirection:'row',alignItems:'center',gap:8},linkedNoticeText:{fontSize:11,fontWeight:'800',color:theme.green2,flex:1}});
