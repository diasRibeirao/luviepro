...
export default function Calendar(){
 ...
 return <AppShell title="Agenda" subtitle="Compromissos, projetos e prazos em um único lugar" action={<HeaderAction label="Novo evento" icon="calendar-outline" onPress={openNew}/> }>
  ...
  <View style={s.filters}>{['Todos',...Object.values(types)].map(x=><Pressable key={String(x)} onPress={()=>setFilter(String(x))} style={[s.filter,filter===String(x)&&s.filterOn,filter===String(x)&&{backgroundColor:brand.primary,borderColor:brand.primary}]}><Text style={[s.filterText,filter===String(x)&&s.filterTextOn,filter===String(x)&&{color:brand.primaryForeground}]}>{x}</Text></Pressable>)}</View>
  <View style={[s.calendar,compact&&s.calendarCompact]}>
   <View style={[s.monthHead,compact&&s.monthHeadCompact,phone&&s.monthHeadPhone]}>
    <View style={s.monthNavigation}>
     <Pressable accessibilityLabel="Mês anterior" onPress={()=>setMonth(new Date(y,m-1,1))} style={s.monthArrow}><Ionicons name="chevron-back" size={20} color={brand.primary}/></Pressable>
     <Text numberOfLines={1} style={[s.month,phone&&s.monthPhone]}>{month.toLocaleDateString(locale,{month:'long',year:'numeric'})}</Text>
     <Pressable accessibilityLabel="Próximo mês" onPress={()=>setMonth(new Date(y,m+1,1))} style={s.monthArrow}><Ionicons name="chevron-forward" size={20} color={brand.primary}/></Pressable>
    </View>
    <Pressable onPress={()=>{const d=new Date();setMonth(d);setSelected(d.toISOString().slice(0,10))}} style={[s.today,phone&&s.todayPhone,{borderColor:brand.primary}]}><Text style={[s.todayText,{color:brand.primary}]}>Hoje</Text></Pressable>
   </View><View style={s.week}>{['SEG','TER','QUA','QUI','SEX','SÁB','DOM'].map(x=><Text key={String(x)} style={s.weekText}>{x}</Text>)}</View><View style={s.grid}>{days.map((d,i)=>{const iso=d?`${y}-${String(m+1).padStart(2,'0')}-${String(d).padStart(2,'0')}`:'';const count=events.filter(e=>occursOn(e,iso)).length;return <Pressable key={i} onPress={()=>d&&setSelected(iso)} style={[s.cell,compact&&s.cellCompact,phone&&s.cellPhone,iso===selected&&s.cellOn,iso===selected&&{backgroundColor:brand.primarySoft,borderColor:brand.primary}]}><Text style={[s.cellText,phone&&s.cellTextPhone,iso===selected&&{color:brand.primary,fontWeight:'900'}]}>{d||''}</Text>{count>0&&<View style={[s.mark,phone&&s.markPhone,{backgroundColor:brand.secondary}]}><Text style={[s.markText,{color:brand.secondaryForeground}]}>{count}</Text></View>}</Pressable>})}</View></View>
  {loading||error?<AsyncState loading={loading} error={error} onRetry={load}/>:<View style={[s.dayPanel,compact&&s.dayPanelCompact]}><Text style={s.dayTitle}>{new Date(selected+'T12:00:00').toLocaleDateString(locale,{weekday:'long',day:'2-digit',month:'long'})}</Text>{dayEvents.length?dayEvents.map(e=><Pressable style={[s.event,compact&&s.eventCompact,phone&&s.eventPhone]} key={e.id} disabled={!e.projectId} onPress={()=>e.projectId&&router.push(`/projects/${e.projectId}` as Href)}><View style={[s.eventIcon,{backgroundColor:brand.primarySoft}]}><Ionicons name={e.type==='project'?'briefcase-outline':'time-outline'} size={18} color={brand.primary}/></View><View style={s.eventContent}><Text style={s.eventTitle}>{e.title}</Text><Text style={s.eventMeta}>{e.allDay?`${isoDate(e.startAt)===isoDate(e.endAt)||!e.endAt?'Dia inteiro':`${new Date(e.startAt).toLocaleDateString(locale,{day:'2-digit',month:'2-digit'})} a ${new Date(e.endAt).toLocaleDateString(locale,{day:'2-digit',month:'2-digit'})}`}`:new Date(e.startAt).toLocaleTimeString(locale,{hour:'2-digit',minute:'2-digit'})}{e.location?` · ${e.location}`:''}{e.projectId&&projectById.get(e.projectId)?.assignee?.name?` · Responsável: ${projectById.get(e.projectId)?.assignee?.name}`:''}{e.projectId?' · Abrir projeto':''}</Text></View><Text style={[s.eventType,compact&&s.eventTypeCompact,phone&&s.eventTypePhone,{color:brand.primary}]}>{types[e.type]||'Evento'}</Text></Pressable>):<Text style={s.empty}>Nenhum compromisso neste dia.</Text>}</View>}
  <FormModal visible={open} title="Novo evento" subtitle="Vincule um projeto para preencher os dados automaticamente ou cadastre um compromisso manual." onClose={close} footer={<><Pressable style={s.cancel} onPress={close}><Text>Cancelar</Text></Pressable><Pressable style={shellStyles.button} disabled={saving} onPress={save}><Text style={shellStyles.buttonText}>{saving?'Salvando...':'Criar evento'}</Text></Pressable></>}>
   ...
   {!form.allDay?<View style={[s.row,compact&&s.rowCompact]}><TimeField label="INÍCIO" required value={form.start} error={formErrors.start} onChange={start=>change('start',start)}/><TimeField label="FIM" required value={form.end} error={formErrors.end} minTime={form.date===form.endDate?form.start:undefined} onChange={end=>change('end',end)}/></View>:<View style={[s.linkedNotice,{backgroundColor:brand.primarySoft}]}><Ionicons name="briefcase-outline" size={17} color={brand.primary}/><Text style={[s.linkedNoticeText,{color:brand.primary}]}>Evento de projeto em período integral · {form.date} a {form.endDate}</Text></View>}
   ...
   {!form.projectId?<><Text style={s.label}>RECORRÊNCIA</Text><View style={s.filters}>{([['none','Não repetir'],['weekly','Semanal'],['monthly','Mensal']] as const).map(([v,l])=><Pressable key={v} onPress={()=>change('recurrence',v)} style={[s.filter,form.recurrence===v&&s.filterOn,form.recurrence===v&&{backgroundColor:brand.primary,borderColor:brand.primary}]}><Text style={[s.filterText,form.recurrence===v&&s.filterTextOn,form.recurrence===v&&{color:brand.primaryForeground}]}>{l}</Text></Pressable>)}</View></>:null}
  </FormModal>
 </AppShell>
}
