...
export function CasaNovaScreen(){
 ...
 const bulkQuantityMode=async(isScalable:boolean)=>{
  if(!selectedIds.length||busy)return;
  const selected=items.filter(item=>selectedIds.includes(item.id));
  if(!selected.length)return;
  setBusy(true);
  try{
   await Promise.all(selected.map(item=>casaNovaApi.updateItem(item.id,{
    isScalable,
    quantityOverride:isScalable?null:quantity(item,guests),
   })));
   await load();
   setMessage(`${selected.length} item(ns) atualizado(s) em massa.`);
  }catch(error){setMessage(errorMessage(error))}finally{setBusy(false)}
 };
  ...
  {loading?<AsyncState loading/>:<View style={s.page}>
   <View style={[s.hero,compact&&s.heroCompact,narrow&&s.heroNarrow]}>
    ...
    <View style={[s.guestCard,narrow&&s.guestCardNarrow]}>
     ...
     <View style={[s.guestRow,narrow&&s.guestRowNarrow]}>
      <Pressable
     ...
     <View style={s.guestMetaRow}>
      <View style={s.guestMetaItem}><Ionicons name="person-outline" size={13} color={theme.muted}/><Text style={s.guestMetaText}>Mínimo: <Text style={s.guestMetaStrong}>2</Text></Text></View>
      <View style={s.guestMetaDivider}/>
      <View style={s.guestMetaItem}><Ionicons name="people-outline" size={13} color={theme.muted}/><Text style={s.guestMetaText}>Máximo: <Text style={s.guestMetaStrong}>999.999</Text></Text></View>
     </View>
    </View>
   </View>
...
const s=StyleSheet.create({
 page:{gap:18},hero:{backgroundColor:theme.green,borderRadius:20,padding:28,flexDirection:'row',gap:24,alignItems:'center'},heroCompact:{flexDirection:'column',alignItems:'stretch'},heroNarrow:{padding:18,borderRadius:16,gap:18},heroText:{flex:1},kicker:{fontSize:10,fontWeight:'900',letterSpacing:1.5,color:theme.goldLight},heroTitle:{fontFamily:'serif',fontSize:30,fontWeight:'800',lineHeight:36,color:theme.white,marginTop:8,maxWidth:620},heroDesc:{fontSize:13,lineHeight:20,color:'rgba(255,255,255,.72)',marginTop:10,maxWidth:620},guestCard:{minWidth:260,borderRadius:16,backgroundColor:theme.white,padding:18},guestCardNarrow:{minWidth:0,width:'100%',padding:14},guestLabel:{fontSize:12,fontWeight:'800',color:theme.ink},guestRow:{flexDirection:'row',alignItems:'center',justifyContent:'center',gap:12,marginTop:12},guestRowNarrow:{gap:8},guestCircleButton:{width:42,height:42,borderRadius:21,alignItems:'center',justifyContent:'center',flexShrink:0},guestCircleButtonSecondary:{backgroundColor:'#F1F5F2',borderWidth:1,borderColor:theme.border},guestCircleButtonPrimary:{backgroundColor:theme.green2},round:{width:42,height:42,borderRadius:21,alignItems:'center',justifyContent:'center',backgroundColor:theme.green50},roundActive:{backgroundColor:theme.green2},roundDisabled:{opacity:.38},guestHeader:{flexDirection:'row',alignItems:'center',gap:10,marginBottom:18},
...
roundPressed:{opacity:.8,transform:[{scale:.97}]},
 summary:{flexDirection:'row',gap:12},summaryCompact:{flexDirection:'column'},summaryNarrow:{gap:9},sumCard:{flex:1,borderRadius:15,borderWidth:1,borderColor:theme.border,backgroundColor:theme.white,padding:18,flexDirection:'row',alignItems:'center',justifyContent:'space-between'},sumDark:{backgroundColor:theme.green,borderColor:theme.green},sumGold:{backgroundColor:theme.goldPale,borderColor:'#EAD99B'},sumLabel:{fontSize:11,fontWeight:'700',color:theme.muted},sumValue:{fontFamily:'serif',fontSize:27,fontWeight:'800',color:theme.ink,marginTop:4},sumDarkText:{color:theme.white},
 body:{flexDirection:'row',alignItems:'flex-start',gap:18},bodyCompact:{flexDirection:'column'},listCol:{flex:1,minWidth:0,width:'100%'},listHeader:{flexDirection:'row',alignItems:'flex-end',justifyContent:'space-between',gap:12,flexWrap:'wrap'},listHeaderNarrow:{alignItems:'stretch'},headerActions:{flexDirection:'row',gap:8,flexWrap:'wrap'},headerActionsNarrow:{flexDirection:'column',alignItems:'stretch'},addItemShortcut:{flexDirection:'row',alignItems:'center',gap:6,backgroundColor:theme.green2,borderRadius:999,paddingHorizontal:14,paddingVertical:10},addItemShortcutText:{fontSize:11,fontWeight:'900',color:theme.white},eyebrow:{fontSize:10,fontWeight:'900',letterSpacing:1.3,color:theme.gold},sectionTitle:{fontFamily:'serif',fontSize:22,fontWeight:'800',color:theme.ink,marginTop:4},essentialBtn:{flexDirection:'row',alignItems:'center',gap:6,backgroundColor:theme.green2,borderRadius:999,paddingHorizontal:14,paddingVertical:10},essentialText:{fontSize:11,fontWeight:'900',color:theme.white},exportBtn:{flexDirection:'row',alignItems:'center',gap:6,borderWidth:1,borderColor:theme.borderStrong,backgroundColor:theme.white,borderRadius:999,paddingHorizontal:14,paddingVertical:10},exportText:{fontSize:11,fontWeight:'900',color:theme.green2},
 filters:{flexDirection:'row',gap:7,flexWrap:'wrap',marginVertical:14},filtersNarrow:{gap:6},filter:{borderWidth:1,borderColor:theme.borderStrong,borderRadius:999,paddingHorizontal:12,paddingVertical:8,backgroundColor:theme.white},filterOn:{backgroundColor:theme.green,borderColor:theme.green},filterText:{fontSize:11,fontWeight:'800',color:theme.green2},filterTextOn:{color:theme.white},
 bulkBar:{borderWidth:1,borderColor:theme.border,backgroundColor:theme.white,borderRadius:13,padding:12,marginBottom:10,gap:10},bulkBarNarrow:{padding:10},selectAll:{flexDirection:'row',alignItems:'center',gap:8},selectAllText:{fontSize:11,fontWeight:'900',color:theme.green2},selectedCount:{fontSize:10,fontWeight:'800',color:theme.muted},bulkActions:{flexDirection:'row',alignItems:'flex-end',gap:8,flexWrap:'wrap'},bulkActionsNarrow:{flexDirection:'column',alignItems:'stretch'},bulkSelect:{minWidth:210,flexGrow:1,maxWidth:280},bulkSelectNarrow:{minWidth:0,maxWidth:'100%',width:'100%'},bulkButton:{minHeight:38,borderRadius:9,borderWidth:1,borderColor:theme.borderStrong,backgroundColor:theme.green50,paddingHorizontal:10,alignItems:'center',justifyContent:'center'},bulkButtonText:{fontSize:10,fontWeight:'900',color:theme.green2},bulkDanger:{minHeight:38,borderRadius:9,backgroundColor:theme.danger,paddingHorizontal:11,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:5},bulkDangerText:{fontSize:10,fontWeight:'900',color:theme.white},bulkConfirm:{paddingTop:9,borderTopWidth:1,borderTopColor:theme.border,flexDirection:'row',alignItems:'center',gap:8,flexWrap:'wrap'},bulkConfirmText:{fontSize:11,fontWeight:'800',color:theme.muted},
 cards:{gap:9},item:{backgroundColor:theme.white,borderWidth:1,borderColor:theme.border,borderLeftWidth:4,borderLeftColor:theme.borderStrong,borderRadius:13,padding:13,flexDirection:'row',gap:10},itemPhone:{padding:12,gap:9},itemDone:{opacity:.72,borderLeftColor:theme.success},itemSelected:{borderColor:theme.green2,backgroundColor:'#FBFDFB'},selectBox:{width:22,height:22,borderRadius:6,borderWidth:2,borderColor:'#91A59A',alignItems:'center',justifyContent:'center',flexShrink:0},selectBoxOn:{backgroundColor:theme.green2,borderColor:theme.green2},checkbox:{width:19,height:19,borderRadius:5,borderWidth:2,borderColor:'#91A59A',alignItems:'center',justifyContent:'center'},checkboxOn:{backgroundColor:theme.green2,borderColor:theme.green2},itemBody:{flex:1,minWidth:0},itemTop:{flexDirection:'row',alignItems:'center',gap:10},itemTopPhone:{flexDirection:'column',alignItems:'stretch',gap:12},itemInfo:{flex:1,minWidth:140},itemInfoPhone:{width:'100%',minWidth:0},itemControls:{flexDirection:'row',alignItems:'center',gap:6,flexShrink:0},itemControlsPhone:{width:'100%',justifyContent:'space-between',gap:8},itemActionButtons:{flexDirection:'row',alignItems:'center',gap:2,flexShrink:0},itemName:{fontSize:13,fontWeight:'900',color:theme.ink,lineHeight:18},itemNameDone:{textDecorationLine:'line-through'},itemMeta:{fontSize:11,color:theme.muted,marginTop:3},boughtRow:{flexDirection:'row',alignItems:'center',gap:6,marginTop:7,alignSelf:'flex-start'},boughtText:{fontSize:9,fontWeight:'800',color:theme.muted},qtyControl:{flexDirection:'row',alignItems:'center',gap:4,flexShrink:0},qtyControlPhone:{flex:1,minWidth:0},qtyStep:{width:31,height:31,borderRadius:8,borderWidth:1,borderColor:theme.borderStrong,backgroundColor:theme.white,alignItems:'center',justifyContent:'center'},qtyStepDisabled:{opacity:.35},qtyBox:{borderRadius:10,backgroundColor:theme.goldPale,paddingHorizontal:10,paddingVertical:6,minWidth:118},qtyBoxPhone:{flex:1,minWidth:92,maxWidth:160},qty:{fontSize:12,fontWeight:'900',color:'#6B5521',textAlign:'center'},qtyCaption:{fontSize:9,fontWeight:'700',color:'#8A7747',textAlign:'center',marginTop:1},detailEdit:{padding:7},trash:{padding:7},
