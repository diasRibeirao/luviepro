...
type Props={label:string;value:string;onChange:(value:string)=>void;minTime?:string;error?:string;helper?:string;required?:boolean};
const pad=(value:number)=>String(value).padStart(2,'0');

export function TimeField({label,value,onChange,minTime,error,helper,required=false}:Props){
 ...
 return <View style={s.wrap}><Text style={[s.label,error&&s.labelError]}>{label}{required?<Text style={s.required}> *</Text>:null}</Text><View style={[s.fieldRow,error&&s.fieldRowError]}>
  <TextInput value={value} onChangeText={text=>onChange(mask(text))} placeholder="HH:MM" placeholderTextColor={theme.muted} keyboardType="number-pad" maxLength={5} style={[s.input,error&&s.inputError]}/>
  <Pressable accessibilityLabel={`Selecionar ${label.toLowerCase()}`} onPress={openPicker} style={[s.clockButton,error&&s.clockButtonError]}><Ionicons name="time-outline" size={20} color={error?theme.danger:brand.primary}/></Pressable>
 </View>{error?<Text style={s.fieldError}>{error}</Text>:helper?<Text style={s.helper}>{helper}</Text>:null}<Modal visible={open} transparent animationType="fade" onRequestClose={()=>setOpen(false)}><Pressable style={s.overlay} onPress={()=>setOpen(false)}><Pressable style={s.dialog} onPress={()=>{}}>
