...
import { periodEnd, periodPrice, planRank } from './plan-policy';
...
@Injectable() export class ApiService {
  constructor(private db:PrismaService,private jwt:JwtService,private mail:MailService,@Optional() private redis?:RedisService,@Optional() private billing?:BillingService,@Optional() private subscriptions?:SubscriptionService){}
  ...
  async login(email:string,password:string){
    ...
    if(!u){const platform=await this.db.platformAdmin.findUnique({where:{email:normalized}});if(platform)return this.platformLogin(normalized,password);throw new UnauthorizedException('E-mail ou senha inválidos');}
    const now=new Date();
  ...
  async platformLogin(email:string,password:string){const admin=await this.db.platformAdmin.findUnique({where:{email:email.trim().toLowerCase()}});if(!admin||!admin.active||!await compare(password,admin.passwordHash))throw new UnauthorizedException('E-mail ou senha inválidos');const payload={sub:admin.id,role:'platform_admin',platformAdmin:true};const token=await this.jwt.signAsync({...payload,typ:'access'},{expiresIn:'15m'});const refreshToken=await this.jwt.signAsync({...payload,typ:'refresh'},{secret:this.refreshSecret(),expiresIn:'30d'});await this.db.platformAdmin.update({where:{id:admin.id},data:{lastLoginAt:new Date(),refreshTokenHash:await hash(refreshToken,10)}});return {token,refreshToken,user:{id:admin.id,name:admin.name,email:admin.email,role:'platform_admin'},platform:true};}
