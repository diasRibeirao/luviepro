...
async function bootstrap(){
  ...
  app.use(async(req:any,res:any,next:any)=>{
    ...
    res.setHeader('Retry-After',String(result.retryAfter));return res.status(429).json({statusCode:429,code:'Too Many Requests',message:'Muitas tentativas. Aguarde e tente novamente.',path,timestamp:new Date().toISOString()});
  });
