...
export class BillingService {
  ...
  private validatePaymentMetadata(remote:MercadoPagoPayment,local:LocalPayment){
    const metadata=remote.metadata;if(!metadata||typeof metadata!=='object')return;
    const expected:Record<string,string>={payment_id:local.id,tenant_id:local.tenantId,plan:local.plan,period:local.period,billing_action:local.billingAction};
    for(const [key,value] of Object.entries(expected)){
      const received=metadata[key];
      if(received!==undefined&&received!==null&&String(received)!==String(value))throw new BadRequestException(`Metadado ${key} do pagamento não confere com a cobrança`);
    }
  }
