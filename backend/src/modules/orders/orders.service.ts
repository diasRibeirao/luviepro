...
export class OrdersService {
  ...
  async update(tenantId:string,id:string,data:UpdateOrderDto,actorUserId?:string){
    ...
    const nextPaymentPlan=data.paymentPlan??current.paymentPlan;
    const nextInstallments=nextPaymentPlan==='cash'?1:(data.installments??current.installments);
    if(nextPaymentPlan==='installments'&&nextInstallments<2)throw new BadRequestException('Venda parcelada deve ter ao menos 2 parcelas');
    const order=await this.db.$transaction(async tx=>{if(canceling){const claimed=await tx.order.updateMany({where:{id,tenantId,status:{not:'canceled'}},data:{status:'canceled',canceledAt:now}});if(claimed.count!==1)throw new BadRequestException('Pedido já foi cancelado por outra operação');for(const item of current.items){const product=await tx.product.update({where:{id:item.productId},data:{stockQuantity:{increment:item.quantity}}});await tx.stockMovement.create({data:{tenantId,productId:item.productId,type:'return',quantity:item.quantity,balanceAfter:product.stockQuantity,unitCostCents:item.unitCostCents,reason:`Cancelamento ${current.number}`,referenceType:'order_cancel',referenceId:current.id,actorUserId}})}}
      return tx.order.update({where:{id},data:{...(data.status!==undefined?{status:data.status}:{}),...(data.deliveryMethod!==undefined?{deliveryMethod:data.deliveryMethod}:{}),...(data.paymentPlan!==undefined||data.installments!==undefined?{paymentPlan:nextPaymentPlan,installments:nextInstallments}:{}),...(data.paymentDueAt!==undefined?{paymentDueAt:data.paymentDueAt?new Date(data.paymentDueAt):null}:{}),...(data.notes!==undefined?{notes:data.notes}:{}),...(data.status==='delivered'&&!current.deliveredAt?{deliveredAt:now}:{}),...(data.status&&data.status!=='delivered'?{deliveredAt:null}:{}),...(canceling?{canceledAt:now}:{}),},include:includeOrder});});
  ...
  async addPayment(tenantId:string,id:string,b:CreateOrderPaymentDto,actorUserId?:string){
    ...
    const paidAt=b.paidAt?new Date(b.paidAt):new Date(); const result=await this.db.$transaction(async tx=>{const amountPaidCents=current.amountPaidCents+b.amountCents;const paymentStatus=amountPaidCents>=current.totalCents?'paid':'partial';const claimed=await tx.order.updateMany({where:{id,tenantId,amountPaidCents:current.amountPaidCents,status:{not:'canceled'}},data:{amountPaidCents,paymentStatus,paidAt:paymentStatus==='paid'?paidAt:null}});if(claimed.count!==1)throw new BadRequestException('O saldo deste pedido foi alterado por outra operação. Atualize o pedido e tente novamente.');await tx.orderPayment.create({data:{tenantId,orderId:id,amountCents:b.amountCents,method:b.method||null,notes:b.notes?.trim()||null,actorUserId,paidAt}});return tx.order.findUniqueOrThrow({where:{id},include:includeOrder})});
