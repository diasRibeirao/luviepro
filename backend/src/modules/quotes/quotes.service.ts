  ...
  async decidePublicProposal(token:string,decision:'approved'|'rejected',name:string){
    ...
    if(decision==='approved'){
      await this.db.$transaction(async tx=>{
        const claimed=await tx.quote.updateMany({where:{id:quote.id,tenantId:quote.tenantId,status:'sent',clientDecision:null},data:{status:'approved',approvedAt:now,clientDecision:'approved',clientDecisionAt:now,clientDecisionName:name.trim()}});
        if(claimed.count!==1)throw new ConflictException('Esta proposta já recebeu uma resposta por outra operação');
        await this.reserveProducts(tx,quote.tenantId,quote.id);
        await this.ensureProjectFromQuote(tx,quote);
      });
    }else{
      await this.db.$transaction(async tx=>{const claimed=await tx.quote.updateMany({where:{id:quote.id,tenantId:quote.tenantId,status:'sent',clientDecision:null},data:{status:'rejected',clientDecision:'rejected',clientDecisionAt:now,clientDecisionName:name.trim()}});if(claimed.count!==1)throw new ConflictException('Esta proposta já recebeu uma resposta por outra operação');await this.releaseProducts(tx,quote.tenantId,quote.id);});
    }
