...
export class RegisterDto { @IsString() @MinLength(2) name!: string; @IsString() @MinLength(2) company!: string; @IsEmail() email!: string; @IsString() @MinLength(8) password!: string; @IsOptional() @IsString() phone?: string; @IsOptional() @IsIn(['starter','pro','business']) plan?: string; @IsOptional() @IsIn(['monthly','quarterly','semiannual','annual']) period?: string; }
